...
| `gw-relativ-reference` | `groundwater_obs` relativ mode | 1990–2026 | caveat | 14 | Depth values' reference point per well (kútfej vs ground surface) unverified; VRA publishes no datum per well. Cross-well depth comparison only valid in balti mode (chart 13). | A kútak mélységi értékei eltérő referenciapontra vonatkoznak, a kútak egymással nem hasonlíthatók össze. | `EXTRACTION_GUIDE.md` §14 VRA földalatti |
| `gw-agard3-local-influence` | `groundwater_obs` well 143970 | 1998–2026 | caveat | 14 | Agárd-3.szennyvíztelep well monitors the wastewater facility surroundings; possible local influence on its series. | Az Agárd-3 kút a szennyvíztelep környezetét figyeli, az értékeit helyi hatások érhetik. | `EXTRACTION_GUIDE.md` §14 VRA földalatti |
| `gw-2018-telemetry-switch` | `groundwater_obs` density | 1990–2018 | handled | 13, 14 | Observation density jumps ~2018 (manual weekly → automated multi-daily). Monthly means make the switch visually neutral. | 2018 előtt kézi, azután automata a megfigyelés; ezért havi átlagokat mutatunk. | `EXTRACTION_GUIDE.md` §14 VRA földalatti |
| `gw-vra-last-year-unprocessed` | `groundwater_obs` | last ~1 year | caveat | 13, 14 | VRA portal statement: last ~1 year of series typically unprocessed ("tájékoztató jellegű"). | A földalatti adatok utolsó kb. egy éve még feldolgozatlan, ezért tájékoztató jellegű. | `docs/vra-api.md` license note |
| `lake-splice-2021-2023` | `monthly_station_obs` `atlag_cm` | 2021–2023 | handled | 13 | `atlag_cm` carries no rows for 2021–2023 (yearbook extraction pending). Chart 13 lake series spliced with `daily_obs agard_vizallas` monthly means — same gauge; yearbook monthly mean = mean of the dailies, so the splice is mathematically equivalent. | A tó 2021–2023-as havi vízszintértékei a napi mérések átlagából számolódtak, mert az évkönyv ezen évei még nincsenek feldolgozva. | `docs/climate-charts-plan.md` chart 13 |
